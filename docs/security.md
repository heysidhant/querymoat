...
- A connection is locked to **Read-Only** when any of these hold:
  ...
  - its `environment` is a protected tag (`production`, `prod`, `live` by default; `querydock.readOnlyEnvironments` can add more but never remove these).
...
- **Agents Cannot Unlock Themselves**: Workspace settings can turn `querydock.mcp.readOnly` on but never off. Turning it off in user settings takes effect only after you confirm in a modal dialog, and the confirmation lives in VS Code SecretStorage (OS keychain) rather than in any settings file.
